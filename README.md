...
A slide becomes a test candidate two ways:

- **Explicit test section.** Its propagated `section` names a test bucket — any section whose name matches the whole word `test`/`tests` (`Live tests`, `Flagship tests`, `Upcoming tests`, `Blocked tests`, `Concluded tests`, …), or one of the `... work` variants some decks use for the same buckets (`Live work`, `Upcoming work`, `Concluded work`, …).
- **Background/Hypothesis structure.** Its `detail` carries BOTH a `Background` and a `Hypothesis` subheader (trailing colon and nested placement OK). This catches test slides in a section whose header is missing or worded ambiguously, so long as the slide follows the deck's usual test scaffold. Suppressed when the propagated section is `Appendix` — appendix pages often reproduce the same scaffold as reference material, not as new experiments.

Test candidates are projected onto a fixed table schema instead of being emitted as free-form `detail` in `slides.jsonl`.
...
3. Crops ONE composite image per slide from the rendered slide PNG at the union bbox of every right-side visual shape (pictures + drawn shapes + labels, tables excluded), writes it to `<test-images-dir>/<ppt-file-name>#<slide-num>.png`, and lands the FULL path in the row's `image_path` column so downstream can render the mockup next to the extracted fields. Cropping from the rendered slide preserves overlays (highlight rectangles, arrows) that a raw picture-blob export would drop, and keeps the mockup composition together as one image instead of splitting it per picture. `--test-images-dir` defaults to the shared `S:\UK\Departments\Data\Samsung EO CRO\Historical Tests` folder. If the extractor runs on non-Windows and the target is a Windows-style path, images are saved to `<per-file-dir>` while `image_path` keeps the shared-folder path (so a Windows consumer resolves it after the outputs are synced to the share). If the target is writable but the mkdir fails for another reason, the extractor falls back to `<per-file-dir>` and stores the local path instead. Under `--no-images` (no rendered PNG available) the extractor falls back to saving each right-side picture blob separately as `<ppt-file-name>#<slide-num>_<i>.<ext>` in the same directory.
...
| `image_path`       | list with the saved composite image's FULL path (single entry, `<test-images-dir>\<ppt-file-name>#<slide-num>.png`, kept in Windows backslash form for the default shared folder). Under `--no-images` falls back to a per-picture list like `<test-images-dir>\<ppt-file-name>#042_1.png` |
