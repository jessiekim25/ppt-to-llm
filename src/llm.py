...
- panels: array of the labeled visual blocks on the slide. Use this for:
  ...
  Every panel MUST include BOTH a non-empty `label` AND a `subheader_path`. These are used to build the saved image's filename, so a blank filename means you dropped one of these fields.

  Each panel:
  {
    "label": "<REQUIRED, non-empty. The panel's title/heading exactly as printed on the slide. If the panel has NO printed title (e.g. an untitled photo, mockup, or diagram), you MUST invent a short 2-5 word English descriptor of what the panel shows so the saved image gets a meaningful filename — e.g. \"S26 Ultra front mockup\", \"grid guides diagram\", \"logo lockup swatch\". For a mixed-content table with no separate title, use the concatenated column headers (e.g. \"Logo/lock-up | Name\"). Never return empty string.>",
    ...
    "subheader_path": [ "<outermost subheader title>", ..., "<innermost subheader title>" ]  // REQUIRED. The ordered list of subheader titles (from the `subheaders`/`children` tree above) whose visual area encloses this panel, outermost first. E.g. a diagram sitting inside a "How to build layout:" child of a "4:1 proportion" parent -> ["4:1 proportion", "How to build layout:"]. If the panel visually sits inside ANY subheader's area (even at just one level), you MUST populate this — do not leave it empty in that case. Return [] ONLY when the panel truly sits at slide level (directly under the main slide title with no enclosing subheader).
  }
