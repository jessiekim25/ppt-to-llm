...
- subheaders: array describing every distinct heading + descriptive-text pair on the slide, other than the main slide title itself. A subheader is a SHORT LABEL that introduces a block of descriptive body text OR a nested sub-block. Includes:
  ...
  STRICT RULES:
    ...
    (f) A qualifying subheader (satisfies α/β/γ/δ) NEVER appears as literal text inside another block's `detail`. If you are writing a subheader's `detail` (or the slide-level `detail`) and the next text line is itself a short bold heading followed by its own descriptive text, STOP appending to the current `detail` and open a new entry — either as the next sibling subheader OR as a `children` entry of the current subheader when it visually sits inside the current subheader's area. Concretely: a bold "Overall guidance" line followed by "- <bullet>", "- <bullet>" text is a subheader, not a continuation of the preceding block's body — its title goes into `title` and its bullets into that entry's `detail`, never merged as flat prose into a sibling's or parent's body.
    (g) PARALLEL BODIES INTENTIONALLY REPEAT: two column subheaders side-by-side may carry identical or near-identical body text (e.g. left "Product positioning of S26 Ultra Main KV" and right "Product positioning of S26 Series KV" both explaining the red-line rule). Emit the FULL body under EACH subheader — do NOT drop the second one as "duplicate" and do NOT collapse both into one subheader. Each column stands on its own.
    (h) A subheader whose title ends with ":" ("How to L-shape panels:", "How to build layout:") is a DEFINITIVE body-follows label. The text immediately below or next to it — including any vertical numbered list ("1. …", "2. …") or dash-bulleted list ("- …", "- …") that visually sits under that label — IS that subheader's `detail`. Never emit the label as a subheader with empty `detail`, and never put that following list into slide-level `detail` or into a sibling/parent subheader's `detail`. Slide-level `detail` is reserved for prose that is NOT under any nearby label (intros, footnotes, disclaimers) and never mixes an under-label list together with a footnote.
    (i) NESTED LISTS INSIDE A BULLET stay with their parent. When a subheader's body is a dash-bulleted list and one of those bullets is elaborated by a small numbered sub-list ("1) …", "2) …") sitting visually indented under that bullet, that numbered sub-list is PART of the same subheader's body — do NOT lift it into slide-level `detail` or into a new subheader entry. Emit it inside the same `detail` string, one item per line, right after the bullet it explains.
    (j) VERBATIM PUNCTUATION AND MARKERS. Reproduce source text character-for-character in the fields you emit: never rewrite "1)" as "1." or "1:", never replace ":" with " — " or an em-dash, never swap a hyphen for an en/em dash. Downstream systems compare against the pdfminer source; a punctuation change produces a false duplicate that gets re-appended as a fallback body block and pollutes the output.
